...
from adb_coupang_automation.core.lifecycle import AppLifecycle
from adb_coupang_automation.core.identity import IdentityManager
from adb_coupang_automation.modules.onboarding import OnboardingModule
from adb_coupang_automation.modules.interaction import InteractionModule
from adb_coupang_automation.modules.search_input import SearchInputModule
from adb_coupang_automation.modules.keyboard import KeyboardModule
from adb_coupang_automation.modules.search_results import SearchResultsModule
...
TARGET_PRODUCT_KEYWORD = "식혜"
TARGET_PRODUCT_PRICE = "20,900" # Precise identification

def main():
    parser = argparse.ArgumentParser(description="Coupang App Automation Bot")
    ...
    print("==========================================")
    print(f"  Coupang App Automation Bot")
    ...
    for cycle in range(args.steps):
         ...
         # Phase 2: App Launch
         print("\n[Phase 2] App Launch (with Data Clear)...")
         
         # Ensure unlocked (Important after reboot)
         interaction.wake_and_unlock()
         
         # Clear Data before launch
         lifecycle.clear_data()
         
         # [New] Pre-grant permissions to skip popups
         lifecycle.grant_permission("android.permission.POST_NOTIFICATIONS")
         ...
         # Scroll Loop (Skipped Naver logic)
         # interaction.run_scroll_and_click()

         # Phase 5: Search Input
         print("\n[Phase 5] Search Input...")
         
         # 1. Enter Search Mode from Main Screen
         print("[Action] Clicking Main Search Bar to enter Search Mode...")
         # Try identifiers found in inspector (Robust Wait)
         search_bar_found = False
         for _ in range(5): # Wait up to 10s (5 * 2s)
             if interaction.tap_element(identifier='resource-id="com.coupang.mobile:id/edit_text"'):
                 search_bar_found = True
                 break
             if interaction.tap_element(identifier='text="쿠팡에서 검색하세요!"'):
                 search_bar_found = True
                 break
             if interaction.tap_element(identifier='resource-id="com.coupang.mobile:id/header_search_input"'): # Another common ID
                 search_bar_found = True
                 break
             
             print("[Search] Waiting for Search Bar to render...")
             time.sleep(2)
             
         if not search_bar_found:
             print("[Warning] Could not find specific search bar after wait. Tapping generic top area...")
             interaction.tap(500, 300) # Generic top area fallback
         
         time.sleep(1.5) # Wait for Search Activity to load (Reduced from 3s)
         ...
         if entered_search:
             ...
             if serp_verified:
                 # [User Request] Skip random browsing; Check Price
                 # print('[Action] Randomized Browsing on SERP...')
                 # search_results.perform_browsing_action()
                 
                 print(f'[Action] Finding Target Product: {finding_keyword} (Price: {TARGET_PRODUCT_PRICE})...')
                 # Uses scan_for_product for scroll-and-search with Price Filter
                 target_bounds = search_results.scan_for_product(
                     finding_keyword, 
                     price=TARGET_PRODUCT_PRICE, 
                     max_scrolls=10
                 )
                 ...
                 if target_bounds:
                     ...
                     # Note: is_product_page needs check for Coupang ("구매하기" etc)
                     if search_results.is_product_page():
                         ...
                         print('[Failure] Did not detect Product Page (Check is_product_page logic).')
                 else:
