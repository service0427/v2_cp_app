...
class DeviceProfile:
    ...
    def get_coupang_app_header(self) -> str:
        ...
        parts = [
            ...
            "Y",                    # 7
            self.model,             # 8
            uuid_no_dash,           # 9 (Verified: match with getEncodedAndroidId/MD5+UUIDv3 logic)
            self.app_session_id,    # 10 (ad_track_id 위치에 app_session_id)
            self.dpi_level,         # 11
            self.pcid,              # 12 ★ PCID (x-signature 계산에 사용)
            "",                     # 13
            "0",                    # 14
            "",                     # 15 (Verified: getMemberSrl returns empty for guest)
            "wifi",                 # 16
            "-1",                   # 17
            "",                     # 18
            "",                     # 19
            "Asia/Seoul",           # 20
            "",                     # 21 (Verified: getSid() returns null/empty, NOT device_hash)
            "",                     # 22
            str(self.width),        # 23
            self.dpi,               # 24
            "-1",                   # 25
