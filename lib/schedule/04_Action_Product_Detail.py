...
def run(session: requests.Session, context: dict = None):
    ...
    if not p_id or not item_id or not srp_pv_id:
        ...
        error_msg = f"[145] Critical Error: Missing context for Product Detail (pId={p_id}, itemId={item_id}, srp_pvId={srp_pv_id}). Enforcement: Fail-Fast."
        log_error("04_Product_Detail", error_msg, context)
    ...
    if response and response.status_code == 200:
        try:
            ...
            # [NEW] Recursive Search specifically for 'Add to Cart' Schemas (ID: 10)
            def find_atc_schemas_blob(obj):
                if isinstance(obj, dict):
                    # Check if this dict has 'clickSchemas'
                    if 'clickSchemas' in obj and isinstance(obj['clickSchemas'], list):
                        schemas = obj['clickSchemas']
                        # Check if this list contains Schema 10 (Add to Cart)
                        # We convert the list to a dict {id: mandatory} for easier use
                        schema_map = {}
                        has_atc_event = False
                        for s in schemas:
                            if 'id' in s and 'mandatory' in s:
                                s_id = str(s['id'])
                                schema_map[s_id] = s['mandatory']
                                if s_id == '10':
                                    has_atc_event = True
                    ...
                    for k, v in obj.items():
                        res = find_atc_schemas_blob(v)
                        if res: return res
                        
                elif isinstance(obj, list):
                    for item in obj:
                        res = find_atc_schemas_blob(item)
                        if res: return res
                return None

            # Extract sdpVisitKey using a simple recursive search (re-implementing simple find for it)
            def find_visit_key(obj):
                if isinstance(obj, dict):
                    if 'sdpVisitKey' in obj and obj['sdpVisitKey']: return obj['sdpVisitKey']
                    for v in obj.values():
                        res = find_visit_key(v)
                        if res: return res
                elif isinstance(obj, list):
                    for item in obj:
                        res = find_visit_key(item)
                        if res: return res
                return None

            sdp_visit_key = find_visit_key(data)
            if sdp_visit_key:
                ...
                from lib.logger import log_error
                error_msg = "[04] Warning: sdpVisitKey not found in SDP Response."
                print(error_msg)
                log_error("04_Product_Detail", error_msg, context)

            # [NEW] Capture Schema Blobs (clickSchemas for ATC)
            atc_schemas = find_atc_schemas_blob(data)
            if atc_schemas:
                print(f"[04] Extracted sdp_atc_click_schemas: Found {len(atc_schemas)} schemas (IDs: {list(atc_schemas.keys())})")
                if 'RESULT' in context and 'ROOT' in context['RESULT']:
                    # We store it under the SAME key 'sdp_atc_click_schemas' so 06 doesn't need changes
                    context['RESULT']['ROOT']['sdp_atc_click_schemas'] = atc_schemas
            else:
                print("[04] Warning: sdp_atc_click_schemas (ID=10) NOT found. 'Add To Cart' will use fidelity fallback.")
