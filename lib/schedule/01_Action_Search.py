...
def run(session: requests.Session, context: dict = None):
    ...
    device = DeviceProfile(
        ...
        member_srl=context['DEVICE'].get('member_srl', ''), # [Fix] Pass member_srl from context
        ...
        try:
            ...
            if entity_list:
                ...
                if not selected_product:
                    ...
                    error_msg = f"[128] Target product {context['INPUT']['productId']} NOT found in search results. Stopping execution."
                    log_error("01_Search", error_msg, context)
            ...
            if selected_product:
                ...
                for i, entity_wrapper in enumerate(entity_list[:end_rank]):
                    try:
                        ...
                        # [Dynamic Layout] Extract Template Info
                        widget_data = {}
                        if 'widget' in entity:
                             widget_data = entity.get('widget', {}).get('data', {})

                        current_template = widget_data.get('currentTemplate', 'LIST')
                        template_map = widget_data.get('template', {})
                ...
                try:
                    if lib.logger.LOG_BASE_DIR:
                        log_file = os.path.join(lib.logger.LOG_BASE_DIR, "PRODUCT.log")
            ...
            # keywordType: 값이 있으면 사용, 없으면 빈문자열 (빈값이 정상 케이스임)
            # 2026-01-14: User request - 'OTHER' is invalid, must be empty string
            if found_keyword_type and str(found_keyword_type).upper() != 'OTHER':
                extracted_data['keywordType'] = str(found_keyword_type)
            else:
                extracted_data['keywordType'] = ''  # 빈값이 유효함
