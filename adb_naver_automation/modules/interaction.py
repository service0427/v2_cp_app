...
class InteractionModule(AutomationEngine):
    ...
    def get_element_center(self, content, resource_id):
        """Parses XML content to find bounds of a resource_id and returns center (x, y)."""
        try:
            # Regex to find the node with the resource_id and capture its bounds
            # Pattern: resource-id="THIS_ID" ... bounds="[x1,y1][x2,y2]"
            # A bit loose regex to match attributes in any order
            import re
            
            # Simplified approach: Extract all nodes, find the one with matching ID
            # But we have the full content string.
            # Let's search for the substring resource-id="ID" and then find bounds nearby?
            # XML parsing is safer.
            import xml.etree.ElementTree as ET
            root = ET.fromstring(content)
            
            for node in root.iter():
                if node.attrib.get('resource-id') == resource_id:
                    bounds = node.attrib.get('bounds') # "[0,116][1080,290]"
                    if bounds:
                        # Parse [x1,y1][x2,y2]
            ...
            print(f"[Error] Failed to parse element center: {e}")
            return None

    def tap_element(self, resource_id, fallback_x=None, fallback_y=None):
        """Taps an element by Resource ID. Uses fallback if not found."""
        self.dump_ui()
        content = self.get_ui_content()
        
        center = self.get_element_center(content, resource_id)
        
        if center:
            cx, cy = center
            print(f"[Automator] Found {resource_id} at ({cx}, {cy}). Tapping...")
            self.tap(cx, cy)
            return True
        elif fallback_x and fallback_y:
            print(f"[Automator] Element {resource_id} NOT found. Using fallback ({fallback_x}, {fallback_y}).")
            self.tap(fallback_x, fallback_y)
            return True
        else:
            print(f"[Automator] Element {resource_id} NOT found and no fallback provided.")
            return False
